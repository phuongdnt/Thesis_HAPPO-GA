"""
train_pareto.py - Serial Topology Pareto Training
==================================================
HAPPO + GA Hybrid with Multi-Objective Pareto Analysis

Usage: python train_pareto.py --config configs/pareto_serial.yaml
"""
...
def parse_config(path: str) -> Dict:
    ...
    if not os.path.isabs(path) or not os.path.exists(path):
        for candidate in [os.path.join(os.getcwd(), path), 
    ...
    # Resolve eval_data_dir path
    env_cfg = cfg.get("env", {})
    eval_data_dir = env_cfg.get("eval_data_dir")
    if eval_data_dir and not os.path.isabs(eval_data_dir):
        for base in [os.getcwd(), os.path.dirname(os.path.abspath(resolved))]:
            candidate = os.path.join(base, eval_data_dir)
            if os.path.exists(candidate):
...
def build_environment(cfg: Dict) -> SerialInventoryEnv:
    """Build serial inventory environment."""
    env_cfg = cfg.get("env", {})
    excluded = ["env_type", "cost_weight", "service_weight", "service_bonus_scale", "agent_num"]
    env_params = {k: v for k, v in env_cfg.items() if k not in excluded}
...
def print_eval_metrics(metrics: Dict, num_agents: int, agent_names: List[str], logger):
    ...
    logger.info(f"  {'Agent':<12} {'Cost':<10} {'FillRate':<10} {'ServiceLv':<10} {'Bullwhip':<10}")
    logger.info(f"  {'-'*52}")
    
    for i in range(num_agents):
        name = agent_names[i] if i < len(agent_names) else f"Agent_{i}"
        logger.info(f"  {name:<12} {metrics['cost_per_agent'][i]:<10.1f} "
    ...
    logger.info(f"  {'-'*52}")
    logger.info(f"  {'TOTAL':<12} {metrics['total_cost']:<10.1f} "
...
def train_single_config(cfg: Dict, cw: float, sw: float, seed: int, 
    ...
    name = f"cw{cw:.1f}_sw{sw:.1f}_seed{seed}"
    exp_log = ParetoLogger(output_dir, name)
    exp_log.log_config({"cost_weight": cw, "service_weight": sw, "seed": seed})
    
    logger.info(f"\n{'='*60}")
    logger.info(f"[SERIAL] cost_weight={cw}, service_weight={sw}, seed={seed}")
    logger.info(f"{'='*60}")
    ...
    logger.info(f"[CONFIG] Backlog costs: {env_cfg.get('backlog_cost', [5, 3, 2])}")
    ...
    episodes = train_cfg.get("episodes", 5000)
    eval_every = train_cfg.get("evaluate_every", 50)
    n_threads = train_cfg.get("n_rollout_threads", 8)
    
    # Early stopping
    es_config = EarlyStoppingConfig(
        enabled=train_cfg.get("early_stop", True),
        patience=train_cfg.get("n_no_improvement_thres", 15),
        min_delta=train_cfg.get("min_improvement_delta", 0.01),
        warmup_evaluations=train_cfg.get("n_warmup_evaluations", 10),
    ...
    if n_threads > 1:
        env = SubprocVecEnv([lambda: build_environment(cfg) for _ in range(n_threads)])
        is_vec = True
    else:
        env = base_env
        is_vec = False
    ...
    checkpoint_every = train_cfg.get("checkpoint_every", 500)  # Save checkpoint every N episodes
    ...
    for ep in range(1, episodes + 1):
        ...
        # Logging
        if ep % 50 == 0:
            logger.info(f"Episode {ep}/{episodes}: reward={ep_reward:.1f}, EMA={smoothed:.1f}")
...
def main(config_path: str):
    ...
    output = pareto.get("output_dir", "results/pareto_serial")
    os.makedirs(output, exist_ok=True)
    
    results = []
    
    logger.info("=" * 60)
    logger.info("PARETO FRONTIER - SERIAL TOPOLOGY")
