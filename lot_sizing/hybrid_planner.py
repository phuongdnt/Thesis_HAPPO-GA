...
Helper class for combining reinforcement learning policies with
heuristic lot-sizing algorithms.  During training or deployment,
an RL agent may propose order quantities that violate operational
constraints (e.g. minimum lot size, fixed cost penalties).  The
:class:`HybridPlanner` intercepts these actions and refines them
using a genetic algorithm heuristic before sending them to the
environment.
"""
    ...
    def refine_actions(self, actions: List[int]) -> List[int]:
        ...
        for i, act in enumerate(actions):
            ...
            if self.use_ga:
                ...
                # DALSA heuristic not implemented; fall back to RL action
                qty = act
            else:
                qty = act
